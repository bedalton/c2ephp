...
class COB {
	...
	public function loadC2COB(IReader $reader) {
		...
		if ($reader->read(4) == 'cob2') {
			while ($reader->hasNext()) {
				if ($failKicks > 4) {
					throw new Exception("Failed to parse COB2. Too many unknown blocks");
				}
				$block = $this->readBlock($reader);
				if (empty($block)) {
					break;
				}
				if ($block instanceof COBUnknownBlock) {
					$failKicks++;
					unset($block);
					continue;
				}
	 ...
	 * @return COBAgentBlock|COBAuthorBlock|COBFileBlock|COBUnknownBlock|null
	 * @throws Exception
	 */
	private function readBlock(IReader $reader) {
		...
		if ($size < 10) {
			throw new Exception("Block byte request for COB agent is too small. Size: $size");
		}
		$contents = $reader->read($size);
		$blockReader = new StringReader($contents);
		switch ($type) {
			case 'agnt':
				//we read the entire thing so that if there are errors we can still go on with the other blocks.
				$block = COBAgentBlock::createFromReaderC2($blockReader);
				break;
			case 'auth':
				$block = COBAuthorBlock::createFromReader($blockReader);
				break;
			case 'file':
				$block = COBFileBlock::createFromReader($blockReader);
				break;
			default:
				...
				$block = new COBUnknownBlock($type, $contents);
				break;
		}
		unset($contents);
		unset($blockReader);
	 ...
	 * @param string|null $format The format of the COB. If null, assumed it's a creatures 2 COB
	 * @return string A binary string containing the COB.
	 * @throws Exception
	 */
	public function compile(?string $format = null) {
		if ($format == null) {
