...
class StreamReader implements IReader {
    ...
    public function __construct($handle) {
        if (!is_resource($handle)) {
			throw new Exception("Handle is not to valid io handle");
		}
        $this->size = -1;
        $this->handle = $handle;
    }
    ...
    public function read(?int $length, bool $cpDecode = self::CP_DECODING_DEFAULT, bool $throwing = TRUE) {
        ...
        if ($this->size < 0 && feof($this->handle)) {
			throw new Exception("Cannot read past end of stream. Expected: $length bytes. Found: 0");
		} else if ($this->getPosition() + $length > $this->size) {
            if ($throwing) {
		...
		if ($str && $cpDecode) {
			$str = EncodingHelper::decodeCreaturesEncoding($str);
		}
        return $str;
    }
